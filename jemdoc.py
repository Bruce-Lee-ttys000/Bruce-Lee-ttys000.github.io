...
class controlstruct(object):
  ...
  def pushfile(self, newfile):
    ...
    self.inf = open(newfile, encoding='utf-8')
...
def parseconf(cns):
  ...
  for sname in cns:
    fs.append(open(sname, encoding='utf-8'))
...
def doincludes(f, l):
  ...
  i = 'include{'
  if l.startswith(ir):
    nf = open(l[len(ir):-2], encoding='utf-8')
...
def geneq(f, eq, dpi, wl, outname):
  ...
  if f.eqcache:
    try:
      dc = open(os.path.join(f.eqdir, '.eqdepthcache'), encoding='utf-8')
      for l in dc:
        a = l.split()
...
def codeblock(f, g):
  ...
  if raw:
    ...
    output,_ = Popen(ext_prog, shell=True, stdin=PIPE, stdout=PIPE,
                     text=True).communicate(buff)
    out(f.outf, output)
  else:
    if g[1] == 'jemdoc':
